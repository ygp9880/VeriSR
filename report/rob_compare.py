...
def compare(meta_path, meta_name, data_path, save_path):
    ...
    save_file_path = os.path.join(save_path, f"{meta_name}_rob2_compare.txt");
    if not 'meta_analysis_tables' in rbo2_json:
        content_utils.write_str_to_file(save_file_path, "[]");
        print("No ROB2 comparison table identified in the source review.");
        return;
    ...
    studies = [];
    for file in files:
        rob2_file = os.path.join(path, file);
