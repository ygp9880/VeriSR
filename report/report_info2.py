...
def extract_report_info(article_text, type, save_path, meta_name):
   ...
   # match_result = json.loads(response_text)
    ...
    file_path = os.path.join(save_path, f"{meta_name}_{type}.json");
...
def data_wrong_summary(base_path, meta_name):
    ...
    type = "report_2_2_3";
    file_path = os.path.join(base_path, f"{meta_name}_{type}.txt");
...
def rob2_summary_report(base_path, meta_name):
    ...
    try:
        compare_json = json.loads(compare_content)
    except Exception:
        compare_json = None

    if compare_json == []:
        response_text = (
            "No ROB2 summary comparison table was identified in the source review, "
            "so agreement between extracted review-level ROB2 judgements and generated study-level ROB2 results "
...
def meta_summary_report(base_path, meta_name):
    file_path = os.path.join(base_path, f"{meta_name}_meta_check_output.json");
