...
def meta_analy(meta_name, data_path, model=DEFAULT_OPENAI_MODEL):
    #meta_name = "SR4";

    base_path = os.path.join(data_path, meta_name);
    ...
    file_path = os.path.join(data_path, f"{meta_name}.txt");
    ...
    #print(f" fig is {figs} ");
    extract_result = check_meta_analysis(figs, "trans", model=model);
    ...
    for item in results:
        ...
        for item in indify_result:
            ...
            meta_study['data_check'] = True;
            if 'study_group' in meta_study:
                input = {'outcome_type': outcome, 'followup_time': followup_time, "study_group": meta_study['study_group']};
            else:
                input = {'outcome_type': outcome, 'followup_time': followup_time};
            if 'mean_experimental' in meta_study:
               ...
               # outcomes_str1 = str(out_comes);
                result = check_outcome(input, original_paper, 'continue', model=model);
                print(f"result is {result} ");
                extraction_correct = result['extraction_correct']
                meta_study['data_check'] = extraction_correct;
               ...
               # out_comes = paper_info['Outcomes']
                ...
                try:
                    result = check_outcome(input, original_paper, 'binary', model=model);
                    #print(f" study_name is {study_name}, data is {data} check outcome is {result}");
                except Exception as e:
                    print("发生异常:", e)
                    result = check_outcome(input, original_paper, 'binary', model=model);
    ...
    save_file_path = os.path.join(data_path, f"extract_result_all_{meta_name}.json");
