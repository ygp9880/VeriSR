...
def parse_llm_json(text: str):
    """Parse JSON from LLM output with light tolerance for wrapped/mixed text."""
    ...
    # Last attempt: grab a likely object block via regex
...
def check_table(meta_path, data_path):
    meta_content,paper_list = meta_read(data_path);
        ...
        for row in rows:
            ...
            if not index_map.__contains__(study):
                ...
                if not check_result:
                    # repeate call
                    indify_result = indify_paper(study, meta_content)
                if not check_index_field(indify_result):
                    index_map[study] = [];
                else:
            ...
            data = f"{headers}\n{row}";
            check = '';
            if len(paper_indexs) == 0:
                check = f"unmatched study: {study}"
            for parper_index in paper_indexs:
                ...
                if not verify_result.__contains__('inconsistent'):
                    ...
                    result_json = parse_llm_json(verify_result);
