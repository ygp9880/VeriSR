import json;
import os
from meta.continue_meta_code import analyze_data;
from utils import content_utils;
import math
from meta.bin_meta_code import meta_item;
#SR3 SR6 SR7
meta_name = "SR1";
file_name = f"extract_result_all_{meta_name}.json";

def preprocess_for_json(data):
    """预处理数据使其适合JSON序列化"""
    if isinstance(data, dict):
        return {k: preprocess_for_json(v) for k, v in data.items()}
    elif isinstance(data, list):
        return [preprocess_for_json(item) for item in data]
    elif isinstance(data, float) and math.isnan(data):
        return None
    elif data in ["False", "True"]:
        return data.lower() == "true"
    else:
        return data


def meta_check(meta_name, data_path):
    file_path = os.path.join(data_path, f"extract_result_all_{meta_name}.json");

    with open(file_path, 'r', encoding='utf-8') as f:
        extract_result = json.load(f)


    count = 0;
    all_result = [];
    for item in extract_result:
        effect_type: str = item['effect_type'];
        title: str = item['title'];
        print(f" title is {title} ");
        studies = item['studies'];
        size = len(studies);
        count = count + 1;
        if size == 0:
            continue;


        study_check = 'events_control' in item['studies'][0];
        if study_check:
            # bin_meta_code 二元结局
            model_check_result = meta_item(item);
            print(f" title is {title}\n data: {item}\n result: {model_check_result}")
            if isinstance(model_check_result, list):
                for result_item in model_check_result:
                    if isinstance(result_item, dict):
                        result_item['title'] = title;
                        all_result.append(result_item);
            else:
                model_check_result['title'] = title;
                all_result.append(model_check_result);
            # 将数据转成 JSON 字符串
            data_to_save = {
                "title": title,
                "data": item,
                "result": model_check_result
            }
            print("已保存到 output.json")

        else:
            # continue_meta_code 连续结局
            png_name = f"{meta_name}_revman_{count}.png";
            calculated_results, validation_results = analyze_data(data_path, item, png_name);
            print(f" title is {title}, {calculated_results}")
            validation_results['title'] = title;
            all_result.append(validation_results);


    cleaned_data = preprocess_for_json(all_result)

    json_str = json.dumps(cleaned_data, default=str, ensure_ascii=False, indent=4)

    # 保存到文件
    output_path = os.path.join(data_path, f"{meta_name}_meta_check_output.json")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(json_str)
