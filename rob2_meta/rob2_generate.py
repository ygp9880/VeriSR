...
def generate(data_path, save_base_path, model=DEFAULT_OPENAI_MODEL):
    base_path = data_path;
    files = os.listdir(data_path);
    rob2_model = model;
    for file in files:
        domain2 = "domain2";
        save_path = os.path.join(save_base_path, file);
        ...
        result:str = extract_rob2_info(content, model=rob2_model);
        ...
        match_1_result = rob2_analysis(content, 'domain1', model=rob2_model);
        logging.info(f" domain1 is {match_1_result} ");
        #match_1_result = None;

        match_2_result = rob2_analysis(content, domain2, model=rob2_model);
        logging.info(f" domain2 is {match_2_result} ");
        match_3_result = rob2_analysis(content, 'domain3', model=rob2_model);
        logging.info(f" domain3 is {match_3_result} ");
        match_4_result = rob2_analysis(content, 'domain4', model=rob2_model);
        logging.info(f" domain4 is {match_4_result} ");
        match_5_result = rob2_analysis(content, 'domain5', model=rob2_model);
...
if __name__ == "__main__":
    ...
    claude_model = DEFAULT_OPENAI_MODEL;
    for file in files:
        domain2 = "domain2";
        ...
        content = read_content(file_path);
        content = content.replace("```json","").replace("```","");
        result:str = extract_rob2_info(content, model=claude_model);
        if result.__contains__('adhering'):
            domain2 = "domain2_deviations";
        #print(f" content is {content} ");
        #match_result = rob2_analysis(content, 'domain2', model="gemini-2.5-pro");
